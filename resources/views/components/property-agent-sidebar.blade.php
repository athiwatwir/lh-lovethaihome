...
<aside class="rounded-2xl border border-gray-200 bg-white p-6 shadow-sm lg:sticky lg:top-24">
    ...
    @if ($user)
        <div class="mt-5 text-center">
            <div class="mx-auto h-28 w-28 overflow-hidden rounded-full border-4 border-blue-100 shadow-md">
                <img
                    src="{{ $user->profileImageOrPlaceholder() }}"
                    alt="{{ $user->fullName() }}"
            ...
            @if ($property->agent['code'] ?? false)
                <p class="mt-1 text-sm text-gray-500">รหัสตัวแทน: {{ $property->agent['code'] }}</p>
            @endif
        </div>
        ...
        <div class="mt-6 space-y-3">
            ...
            @if ($user->id)
                <a
                    href="{{ route('properties.index', ['user_id' => $user->id]) }}"
                    class="flex items-center justify-center gap-2 rounded-xl border-2 border-blue-200 bg-linear-to-r from-blue-50 to-indigo-50 px-4 py-3 text-sm font-semibold text-blue-800 transition hover:border-blue-400 hover:from-blue-100 hover:to-indigo-100"
                >
    ...
    <div class="mt-6 rounded-xl bg-blue-700 px-4 py-4 text-center text-white">
        <p class="text-sm text-blue-100">สนใจทรัพย์นี้?</p>
        <p class="heading-font mt-1 text-lg font-bold">ปรึกษาฟรี ไม่มีค่าใช้จ่าย</p>
    </div>
</aside>
